...
// A-Frame component for hand tracking
export const mediapipeHandComponent = {
  tick: function (time) {
    ...
    // Update pulse rings
    if (this.pulseRings && !this.handMarkersVisible) {
      // Create new pulse if enough time has passed
      if (time - this.lastPulseTime > this.pulseInterval) {
        this.createNewPulse(time)
        this.lastPulseTime = time
      }

      // Update existing pulses
      this.pulseRings = this.pulseRings.filter((pulse) => {
        const progress = (time - pulse.startTime) / 1000 // Convert to seconds
        ...
        pulse.material.uniforms.pulseProgress.value = progress
        return true
      })
    }
  ...
  createHandEntities: function () {
    ...
    for (let h = 0; h < 2; h++) {
      ...
      // Add hand occluder for AR occlusion
      handGroup.setAttribute('hand-occluder', {
        hand: h === 0 ? 'left' : 'right',
        debug: false, // Set to true to see the occlusion geometry
        debugColor: h === 0 ? '#00ff00' : '#0000ff', // Green for left, blue for right
      })
      ...
      for (let i = 0; i < 21; i++) {
        ...
        // Create sphere geometry with normal material
      ...
      connections.forEach(([start, end]) => {
        ...
        line.setAttribute(
          'line',
          `start: 0 0 0; end: 0 0 0; color: #888888; opacity: 0.6`
        )
  ...
  setupEventListeners: function () {
    ...
    this.el.sceneEl.addEventListener('action-menu-clicked', (event) => {
      if (event.detail.action === 'toggle-hand-markers') {
        ...
        this.el.sceneEl.emit('hand-markers-toggled', { visible: this.handMarkersVisible })
        ...
        landmarks.forEach((landmark, i) => {
          if (i < hand.landmarks.length) {
            ...
            if (
              ...
              // Always show index fingertip (landmark 8), hide others if markers disabled
              if (i === 8) {
                hand.landmarks[i].object3D.visible = true

                // Apply special shader when index is isolated
                if (!this.handMarkersVisible) {
                  const meshObject = hand.landmarks[i].getObject3D('mesh')
                  if (meshObject) {
                    // Store original material if not already stored
                    const landmarkKey = `${handIndex}-${i}`
                    if (!this.originalMaterials.has(landmarkKey)) {
                      this.originalMaterials.set(
                        landmarkKey,
                        meshObject.material
                      )
                    }
                    // Apply shader material with current drawing color
                    meshObject.material = this.indexFingerShader
                    this.indexFingerShader.uniforms.color.value =
                      new THREE.Color(this.currentColor)
                    this.indexFingerShader.uniforms.glowColor.value =
                      new THREE.Color(this.currentColor).multiplyScalar(1.5)
                    // Make it slightly larger when isolated
                    meshObject.scale.setScalar(1.5)

                    // Store reference for pulse effect
                    this.currentIndexFinger = {
                      hand: handIndex,
                      landmark: i,
                      entity: hand.landmarks[i],
                    }
                  }
                } else {
                  // Restore original material when markers are visible
                  const meshObject = hand.landmarks[i].getObject3D('mesh')
                  const landmarkKey = `${handIndex}-${i}`
                  if (meshObject && this.originalMaterials.has(landmarkKey)) {
                    meshObject.material =
                      this.originalMaterials.get(landmarkKey)
                    // Restore original scale
                    meshObject.scale.setScalar(1.0)
                  }
                }
              } else if (this.handMarkersVisible) {
                hand.landmarks[i].object3D.visible = true
                // Ensure other markers use original material
                const meshObject = hand.landmarks[i].getObject3D('mesh')
                const landmarkKey = `${handIndex}-${i}`
                if (meshObject && this.originalMaterials.has(landmarkKey)) {
              ...
              if (!this.handMarkersVisible) {
                ...
                if (
                  ...
                  lines[index].setAttribute(
                    'line',
                    `start: ${startPos}; end: ${endPos}; color: #888888; opacity: 0.6`
                  )
                }
              }
            }
  ...
  updateStroke: function () {
    ...
    if (this.currentStrokeEntities.length > 0 && window.DEFAULT_SCENE_NAME === 'webar-hand') {
  ...
  updateExistingStroke: function(entity) {
    ...
    const smoothedPoints = this.strokePoints.length > 10
      ? this.smoothStrokePoints(this.strokePoints)
      : this.strokePoints
  ...
  createNewPulse: function (time) {
    if (!this.currentIndexFinger || !this.currentIndexFinger.entity) return

    // Create ring geometry
    const ringGeometry = new THREE.RingGeometry(0.015, 0.02, 32)

    // Clone material with unique uniforms
    const material = this.pulseMaterial.clone()
    material.uniforms.color.value = new THREE.Color(this.currentColor)
    material.uniforms.pulseProgress.value = 0
    ...
    this.currentIndexFinger.entity.object3D.getWorldPosition(worldPos)
    ...
    const camera = document.querySelector('#camera')
    if (camera) {
      ringMesh.lookAt(camera.object3D.position)
    }

    // Add to scene
    this.el.sceneEl.object3D.add(ringMesh)
  ...
  endStroke: function () {
    ...
    if (this.strokePoints.length > 3 && this.currentStrokeEntities.length > 0) {
      ...
      const strokeEntity = this.currentStrokeEntities[this.currentStrokeEntities.length - 1]
      ...
      this.el.sceneEl.emit('drawing-loop-created', {
        ...
        intersection: intersection || { point: this.strokePoints[0].clone(), index: 0 }, // Always provide intersection
