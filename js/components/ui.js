...
export const uiManagerComponent2 = {
  init() {
    // Attach invisibility-cloak component if not present
    if (!this.el.components['invisibility-cloak']) {
      this.el.setAttribute('invisibility-cloak', '') // defaults
    }

    const inv = () => this.el.components['invisibility-cloak']
    ...
    toggleBtn?.addEventListener('click', () => {
      inv()?.toggleCloak()
      toggleBtn.textContent = inv()?.cloakEnabled ? 'Disable' : 'Enable'
    })

    blur?.addEventListener('input', (e) => inv()?.setBlur(e.target.value))
    mirror?.addEventListener('change', (e) =>
      inv()?.setMirror(e.target.checked)
    )
