...
export const invisibilityComponent = {
  ...
  async _startWhenVideoReady() {
    ...
    // Segmentation loop
    const loop = () => {
      if (this.data.enabled && !this.sending && this.video?.readyState >= 2) {
        this.sending = true
        this.segmenter.send({ image: this.video }).finally(() => {
          this.sending = false
        })
      }
  ...
  captureBackground() {
    ...
    this.bgCtx.save()
    // Don't mirror capture - we'll mirror during playback to match live video
  ...
  _fitOverlayToViewport() {
    ...
    // Reset transform - we'll handle scaling manually in drawing
  ...
  onResults(results) {
    // results.image has same aspect as input video; we'll render to full viewport canvas
...
export const invisibilityCloakComponent = {
  ...
  _processFrame() {
    ...
    if (this.data.enabled && this.cloakEnabled && this.bgReady) {
      ...
      // Create a temporary canvas for the mask at output resolution
      ...
      // Put mask data on temporary canvas at work resolution
      ...
      // Scale mask to match video output
      tempCtx.drawImage(maskCanvas, 0, 0, vw, vh, dx, dy, dw, dh)
